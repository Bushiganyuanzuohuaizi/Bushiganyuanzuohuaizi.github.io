---
title: Algorithms
---

# Data Structures and Algorithms

数据结构与算法笔记，包含基础题型、经典算法、复杂度分析，以及配套代码示例。

## Core Notes

- [C++ Basics](cpp-basics.md)
- [BFS and DFS](bfs and dfs.md)
- [Double Pointer](DoublePointer.md)
- [Sliding Window](滑动窗口.md)
- [Backtracking](回溯算法.md)
- [Hashing](哈希法.md)

## Algorithms and AI

- [Overview](Algorithms&AI/overview.md)
- [Introduction](Algorithms&AI/algoIntro.md)
- [Complexity](Algorithms&AI/Complexity.md)
- [Factorial and Exponential](Algorithms&AI/Factorial&Exponential.md)
- [Sort Algorithms](Algorithms&AI/sortAlgo.md)
- [UFS and IFS](Algorithms&AI/UFS&IFS.md)

## Start Here

- 从 `C++ Basics` 和 `Double Pointer` 开始，适合先热身。
- 然后进入 `BFS and DFS`、`Backtracking`、`Sliding Window`。
- 如果你在做专题整理，再看 `Algorithms and AI` 这一组。

## Code Examples

- `BinaryTree/`
- `Digui/`
- `QuickFind/`
- `StackLearn/`
- `linkedlist/`
- `others/`
- `examples/`
