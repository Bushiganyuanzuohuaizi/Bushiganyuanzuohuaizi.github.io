...
你以后本地新增 `md` 笔记，最简单的人话版就是：

1. 把新文件放到对应文件夹里，比如 `docs/os/`、`docs/ds-algorithm/` 这种。
2. 在 [mkdocs.yml](E:/课件与笔记汇总/DaranDeng.github.io/mkdocs.yml) 里把它加进 `nav`。
3. 文件开头尽量写个简单 front matter，比如：
```md
---
title: 你的标题
---
```
1. 如果是某个新栏目，就先建一个 `index.md` 当栏目首页，再把这个栏目挂进 `nav`。
2. 其他基本不用管，样式会自动吃到我现在这套全站主题。

最省心的理解方式是：
- 新增一篇文章 = 放文件 + 加 `nav`
- 新增一个栏目 = 建文件夹 + 建 `index.md` + 加 `nav`
- 你不用每次改 CSS，除非想做特殊页面
